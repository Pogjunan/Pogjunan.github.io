# -*- coding: utf-8 -*-
"""build.py - 2026 월드컵 페이지 자동 갱신기.

football-data.org에서 경기 결과를 받아 index.html의 동적 영역
(랭킹 상태 태그, 32강 대진/스코어, 생존·탈락 카운트, 16강 확정 목록, 갱신 시각)을 다시 씀.
조별리그 표, 디자인, SEO, 조회수 등 정적 부분은 건드리지 않음.

사용법:
  python build.py                 # FOOTBALL_DATA_KEY 환경변수 사용(운영)
  python build.py --local DIR     # DIR/wc_matches.json 사용(오프라인 테스트)
"""
...
def render_mcard(a, b, date, iso2id, by_pair):
    ...
    if not (x and x["status"] == "FINISHED"):
        return (f'<div class="mt ">'
                f'<div class="mtr ">{flag(a[1], a[0], "")}<span class="nm">{html.escape(a[0])}</span><span class="sc"></span></div>'
                f'<div class="mtr ">{flag(b[1], b[0], "")}<span class="nm">{html.escape(b[0])}</span><span class="sc"></span></div>'
                f'<div class="mmeta">{date} 예정</div></div>')
    sc = x["score"]
...
def main():
    ...
    print(f"OK · 16강 {c.get('r16', 0)} · 32강생존 {c.get('r32', 0)} · "
          f"32강탈락 {c.get('r32out', 0)} · stage {stage}")
