...
(랭킹 상태, 32강 대진/스코어/라이브, 생존·탈락 카운트, 16강 확정, 상태 도크, 경기 일정, 갱신 시각)을 다시 씀.
조별리그 표, 디자인, SEO, 조회수 로직 등은 건드리지 않음.
...
def render_dock(live, today, timed, id2ko):
    if live:
        x = live[0]
        badge = (f'<span class="db live">● LIVE · {STAGE_KO.get(x["stage"], "")} '
                 f'{html.escape(_match_label(x, id2ko))}</span>')
    else:
...
def render_sched(live, timed, id2ko):
    rows = []
    for x in (live + timed)[:8]:
        t = to_kst(x)
        islive = x["status"] in LIVE_STATUS
        istoday = t.date() == NOW.date()
        cls = "live" if islive else ("today" if istoday else "")
        if islive:
            when = "LIVE 진행중"
        elif istoday:
...
def main():
    ...
    print(f"OK · 16강 {c.get('r16', 0)} · 32강생존 {c.get('r32', 0)} · "
          f"32강탈락 {c.get('r32out', 0)} · stage {stage} · 라이브 {len(live)} · 오늘 {len(today)}")
